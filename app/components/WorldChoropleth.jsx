...
export default function WorldChoropleth({ counts, valueLabel = "orders" }) {
  ...
  // Normalise incoming keys to lowercase to match the map's ISO2 ids.
  const byId = useMemo(() => {
    const m = {};
  ...
  // Legend bands: label ranges from the thresholds.
  const legend = [{ color: ZERO_FILL, label: "0" }];
  let lo = 1;
  buckets.forEach((hi, i) => {
    const label = hi === lo ? `${lo}` : `${lo}–${hi}`;
    legend.push({ color: RAMP[i + 1], label });
    lo = hi + 1;
  });
