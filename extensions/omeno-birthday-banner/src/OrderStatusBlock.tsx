...
const STORE_OPTIONS = [
  { value: "", label: "Select your preferred store" },
...
function CustomerBirthdayBlock() {
    ...
    async function fetchCustomerTags() {
      try {
        ...
        if (customer?.tags) {
          ...
          // Find store tag (format: store1, store2, etc.)
          const storeTag = tags.find((tag: string) => 
            STORE_OPTIONS.some(opt => opt.value === tag && opt.value !== "")
          );
          if (storeTag) {
            setSelectedStore(storeTag);
          }
        }
      } catch (e: any) {
  ...
  async function save() {
    ...
    if (!dayVal && !monthVal && !selectedStore) {
    ...
    try {
      ...
      // Add store tag if selected
      if (selectedStore) {
        tagsToAdd.push(selectedStore);
      }
      ...
      // Call app proxy to add the tags (using the region-specific proxy URL)
      ...
      setTimeout(async () => {
        try {
          ...
          if (customer?.tags) {
            ...
            setSaved("Information saved successfully!");
          }
        } catch (e) {
  ...
  return (
    <Banner>
      <BlockStack spacing="base">
        <Text size="large" emphasis="bold">Your Birthday & Preferences</Text>
        <Text>We'll use this information to personalize your experience!</Text>
